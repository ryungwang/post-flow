...
- 공유 postgres `db`(postgres:16)에 **DB `synub_postflow`** + **전용 role `postflow`**. **스키마 `postflow`·테이블은 Flyway가 부팅 시 자동 생성**(`create-schemas: true`, V1~V25) — 사람이 만들 필요 없음.
...
- 빌드타임 env(VITE_*는 번들에 박힘 — 바뀌면 재배포):
  - `VITE_API_BASE_URL=https://postflow-api.synub.io`  ← ⚠️ API는 **별도 서브도메인**(office=office-api). 프론트 도메인(postflow.synub.io)은 Vercel이라 /api 없음
...
- **구독(빌링)**: service_code=`post-flow`. 빌링 카탈로그의 제품 **webhook_url을
  `https://postflow-api.synub.io/webhooks/billing`**로 등록(API 호스트, `/api` 프리픽스 없음). entitlements는 `SYNUB_BILLING_SERVICE_KEY`로 pull.
- **Threads(Meta)**: 앱 Redirect URI에 `https://postflow-api.synub.io/threads/callback` 등록(백엔드 콜백).
