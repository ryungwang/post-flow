...
## 3.6.1 비즈니스 인증 — 데이터 사용 3문항 (인증 제출 화면)

**Q1. 플랫폼 데이터를 어떻게 사용하나요? (텍스트 · 복붙)**

> PostFlow는 크리에이터가 자신의 Threads 계정을 연결해 콘텐츠를 발행·예약하고, 본인 게시물의 성과(인사이트)와 댓글·멘션을 한 곳에서 관리하도록 돕는 콘텐츠 자동화 서비스입니다. Threads에서 얻은 데이터는 오직 연결한 사용자 본인에게 이 기능들을 제공하기 위해서만 사용하며, 타인에게 공유·판매하지 않고 연결을 해제하면 즉시 폐기합니다.

**Q2. 여러 비즈니스 포트폴리오를 관리하나요? → `No`**
- 사용자가 본인 개인 Threads 계정을 직접 연결하는 셀프서비스라, 여러 고객사 포트폴리오를 대신 관리하지 않음. (⚠️ 본인 Business Manager 구성이 실제 다포트폴리오면 Yes)

**Q3. 웹사이트 링크 → `https://postflow.synub.io`**
- 로그인/랜딩 화면에 제품 설명 + 사업자정보(상호·대표·사업자등록번호·통신판매번호) 공개돼 요건 충족. 검수 중 도메인 변경 금지라 현재 도메인 그대로.
...
threads_profile_discovery (설명란)

PostFlow는 크리에이터를 위한 AI 콘텐츠 자동화 서비스입니다. 이 권한은 사용자가 입력한 공개 계정(@아이디)의 프로필과 최근 게시물 성과를 조회해             
경쟁사·벤치마킹 분석을 제공하는 '경쟁사 분석' 기능에 사용됩니다. 공개 계정이면서 팔로워 100명 이상인 계정만 대상으로 하며, 조회한 공개 데이터는 사용자에게
비교·분석 결과를 보여주기 위해서만 사용합니다. 타인 데이터를 저장·판매하지 않고, 사용자가 연결을 해제하면 즉시 폐기합니다.

threads_keyword_search (설명란)

이 권한은 사용자가 입력한 키워드로 지금 인기 있는 공개 게시물을 검색해, 어떤 훅·포맷·주제가 반응이 좋은지 트렌드를 파악하고 AI 콘텐츠 생성에 반영하는 
'트렌드 반영 생성' 기능에 사용됩니다. 검색으로 얻은 공개 데이터는 사용자에게 트렌드 인사이트를 보여주기 위해서만 사용하며, 타인의 데이터를 저장·판매하지 
않습니다.