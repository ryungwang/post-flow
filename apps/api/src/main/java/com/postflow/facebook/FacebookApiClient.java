...
public class FacebookApiClient {
    ...
    /**
     * Query-encode a value. Graph의 중첩 필드 문법 '{}'는 UriComponentsBuilder가 URI 템플릿으로
     * 오인하므로(picture{url} → expand 'url' 오류), 그 경로를 피하고 값을 직접 인코딩해 절대 URI를 만든다.
     */
    ...
    private String base() {
        return properties.graphBaseUrlOrDefault() + ver();
    }
    ...
    public List<FbPage> getPages(String userToken) {
        URI uri = URI.create(base() + "/me/accounts"
                + "?fields=" + enc("id,name,access_token,picture{url}")
    ...
    public List<FbComment> getComments(String postId, String pageToken) {
        URI uri = URI.create(base() + "/" + postId + "/comments"
                + "?fields=" + enc("id,message,from{name}")
