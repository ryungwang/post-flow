...
public class ContentPromptBuilder {
    ...
    public String affiliateUserPrompt(GenerateAffiliateRequest req, int bodyBudget, boolean linkInComment) {
        ...
        // 댓글 모드: 링크·고지문이 첫(고정) 댓글로 나간다 → 본문은 재미·궁금증으로 몰입시키고 댓글을 보게 유도.
        String closing = linkInComment
                ? "궁금증을 최고조로 남긴 채 자연스럽게 '고정(첫) 댓글'을 보게 유도하며 끝내라 — 정보·링크가 댓글에 있음을 넌지시."
                : "부담 없는 추천으로 마무리.";
        String ctaRule = linkInComment
                ? "- cta 는 링크가 아니라 '고정(첫) 댓글'을 보게 유도 — 예: \"링크는 고정 댓글에\", \"최저가·품번은 첫 댓글 확인\", \"댓글에 정리해뒀어요\". 궁금해서 댓글을 누를 수밖에 없게."
                : "- cta 는 부담 없는 추천 한 줄(예: \"필요하면 링크로 확인해 보세요\", 이미지 중심 플랫폼이면 \"프로필 링크에서 확인\").";
        return """
                제품(쿠팡파트너스 제휴 게시물): %s
                %s
                톤: %s

                이건 제휴 게시물이다. SNS에서 스크롤을 멈추게 하는 재밌고 몰입되는 글을 써라:
                1) 첫 줄은 강한 훅 — 공감·호기심·반전으로 스크롤을 멈추게 (흔한 실수·의외의 사실·"이거 모르고 샀다가…" 류).
                2) 문제·상황에 확 공감 → 3) 이 제품이 왜 해결인지(구체적이되 정직하게) → 4) %s

                몰입 규칙(재밌고 자극적이되 정직하게):
                - 호기심 갭·공감·반전·구체 디테일로 끝까지 읽게 만들어라. 밋밋한 스펙 나열 금지.
                - '자극적'은 앵글·훅으로만 낸다. 허위·과장·낚시성 거짓은 절대 금지.

                엄격 규칙(반드시 지켜라):
                - 확인되지 않은 스펙·가격·할인·수치·효과·후기·"내가 써봤다/내돈내산" 날조 금지.
                - "1위·무조건·최저가 보장" 같은 근거 없는 단정 금지. "광고 아님"처럼 대가성 숨기는 표현 금지.
                - 본문(content)·cta 어디에도 URL·고지문·이미지태그를 넣지 마라. 링크·고지문은 시스템이 붙인다.
                - 각 content 는 %d자 이내. 자연스럽게 끝나게.
                %s

                %d개의 서로 다른 각도의 게시물을 JSON 배열로.
                """.formatted(
                req.productName(), featuresLine, req.toneOrDefault(),
                closing, bodyBudget, ctaRule, req.quantity());
    }
