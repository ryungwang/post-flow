...
public class ContentPromptBuilder {
    ...
    public String affiliateUserPrompt(GenerateAffiliateRequest req, int bodyBudget, boolean linkInComment) {
        ...
        // 특장점은 '이 제품이 어떤 상황·니즈를 위한 건지' 파악하는 용도일 뿐 — 본문엔 절대 등장하지 않는다.
        String featuresLine = features == null
                ? "특장점 정보 없음 → 이 제품이 어떤 상황·고민에 쓰이는지만 감 잡아, 그 '상황'을 일반적으로 써라."
                : "제품 특장점(이걸로 '이 제품이 해결하는 상황·니즈'만 파악하라. 특징·스펙·기능·셀링포인트는 본문에 절대 쓰지 마라): " + features;
        // 본문엔 제품이 아예 안 나온다. 홍보(제품+링크)는 오직 댓글로 — 링크·고지문은 시스템이 붙인다.
        return """
                제품(쿠팡파트너스 제휴 — 참고 맥락일 뿐, 본문에 등장 금지): %s
                %s

                본문(content)은 '제품 소개·홍보'가 절대 아니다. 이 제품이 필요해지는 '상황·고민·니즈'에 대해
                사람들이 공감하거나 궁금해할, 제품 없이도 성립하는 일반적인 반말 글이다. 홍보는 본문이 아니라 댓글(링크)로만.

                *** 철칙 — 어기면 그냥 광고다 ***
                - 본문에 제품·브랜드·모델명·기능·스펙·셀링포인트를 언급하지 마라. "이거/이런 거"로 가리키지도 마라.
                  제품은 본문에 아예 안 나온다.
                - 특장점을 '팁'으로 바꿔 나열하지 마라. (홈멀티·AI절약·보증연장·방문설치 같은 셀링포인트를
                  '정보/팁'인 척 풀면 그게 결국 광고다.) 대신 그 제품이 필요한 '상황·불편·궁금증'만 그려라.
                예) 제품이 '거실+방 같이 쓰는 홈멀티 에어컨'이면 →
                    본문: "거실은 시원한데 방은 안 식어서 여름밤마다 잠 설치는 집 많잖아" 같은 상황 공감(제품·기능 언급 X).
                    → 그 상황이 궁금한 사람이 알아서 댓글(링크)을 본다.

                톤·구성:
                - 무조건 반말, 담담하게. 친구한테 툭 던지듯. 존댓말·과장 간증·"내돈내산" 가짜후기 금지.
                - 첫 줄은 공감·의외의 관찰로 스크롤을 멈추게("아 맞아" 하게).
                - 끝맺음은 '상황 묘사 + 참여를 부르는 질문' 한 줄로. "너네 집도 그렇지 않아?", "나만 이래?",
                  "이런 적 없어?", "다들 겪지 않아?"처럼 부드럽게 공감·댓글을 부르게.
                  ⚠️ "~냐?"(없냐/않냐) 같은 거친 말투는 쓰지 마라. "~어?/아?"로 부드럽게.
                - "살 때 챙겨라 / 안 하면 후회함 / 이게 이득" 같은 조언·훈수·구매 유도 톤 금지(그게 광고 냄새다).
                - 해시태그 0~2개, 일상 톤만. 브랜드·제품·구매 태그(#삼성에어컨 #에어컨추천 등) 절대 금지.

                형식:
                - 본문·cta 에 URL·고지문·이미지태그·"댓글/링크 확인" 유도 금지. cta 는 비워라("").
                - 없는 사실·수치 날조 금지. 각 content 는 %d자 이내.

                %d개의 서로 다른 '상황·앵글'의 게시물을 JSON 배열로.
                """.formatted(
